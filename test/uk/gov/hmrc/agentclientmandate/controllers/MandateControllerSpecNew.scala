...
import uk.gov.hmrc.agentclientmandate.auth.{AuthFunctionality, AuthRetrieval}
...
class MandateControllerSpecNew extends PlaySpec with MockitoSugar with BeforeAndAfterEach  with GuiceOneServerPerSuite {
  ...
  class Setup {
    val TestMandateController = new MandateControllerNew(
      createServiceMock,
      updateServiceMock,
