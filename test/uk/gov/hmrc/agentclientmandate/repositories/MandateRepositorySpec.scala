...
//import uk.gov.hmrc.agentclientmandate.models.{ContactDetails, GGRelationshipDto, MandateStatus, Party, PartyType, Service, Status, Subscription, User}
//import uk.gov.hmrc.agentclientmandate.repositories.{MandateFetched, MandateMongoRepository}
...
//
//class MandateRepositorySpec extends PlaySpec with MongoSpecSupport with BeforeAndAfterEach with MockitoSugar with MustMatchers {
...
//
//    "return a Mandate Create Error" when {
//
...
////  }
////
// def testMandateRepository(implicit mongo: () => DB) = new MandateMongoRepository(mongo)
//
//  def mandate: Mandate =
...
//  mock[GenericCollection[JSONSerializationPack.type]]
//
...
//  }
//
//  class TestMandateRepository(implicit mongo: () => DB) extends MandateMongoRepository(mongo) {
