...
trait MandateFetchService {
  ...
  def fetchClientMandate(mandateId: String)(using ec: ExecutionContext): Future[MandateFetchStatus] = {
    mandateRepository.fetchMandate(mandateId)
  }

  def fetchClientMandate(clientId: String, service: String)(using ec: ExecutionContext): Future[MandateFetchStatus] = {
  ...
  def getAllMandates(arn: String, serviceName: String, credId: Option[String], displayName: Option[String])
                    (using ar: AuthRetrieval, ec :ExecutionContext): Future[Seq[Mandate]] = {
    if (credId.isDefined) {
  ...
  def getMandatesMissingAgentsEmails(arn: String, service: String)(using ec: ExecutionContext): Future[Seq[String]] = {
    mandateRepository.findMandatesMissingAgentEmail(arn, service)
  }

  def fetchClientCancelledMandates(arn: String, serviceName: String)(using ec: ExecutionContext): Future[Seq[String]] = {
